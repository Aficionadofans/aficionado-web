  ...
  if (!isSubscribed) {
    return (
      <div className="relative w-full h-[75dvh] flex flex-col items-center justify-center p-6 overflow-hidden">
        ...
        <div
          className="absolute inset-0 z-0 flex flex-col gap-6 opacity-30 blur-[40px] pointer-events-none p-4"
          aria-hidden="true"
        >
          ...
          <div
            className="bg-white/20 rounded-3xl h-24 w-2/3 animate-breathe-calm"
        ...
        <div className="relative z-10 liquid-glass p-8 rounded-[2rem] max-w-sm w-full text-center border border-amber-500/30 animate-fade-in-up shadow-2xl">
          ...
          <p className="text-white/70 mb-8 text-xs sm:text-sm leading-relaxed text-pretty">
            Unlock exclusive behind-the-scenes content, direct chat, and VIP co-watching with @
            {username}.
          </p>
    ...
    <div className="w-full min-h-[75dvh] flex flex-col bg-background relative overflow-hidden">
      ...
      >
        ...
        <div className="liquid-glass p-5 rounded-2xl border-amber-500/30 mb-6 relative overflow-hidden animate-fade-in-up flex-shrink-0 shadow-lg">
          ...
          <h3 className="text-lg font-bold text-amber-400 flex items-center gap-2 mb-1.5 relative z-10">
            <Star className="w-5 h-5 fill-amber-400 text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.8)]" />{' '}
            VIP Lounge
          </h3>
          <p className="text-xs sm:text-sm text-white/85 relative z-10 leading-relaxed text-pretty max-w-[90%]">
            You&apos;re in! Thanks for supporting @{username}. This space is for exclusive drops and
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      isCreator
                        ? 'border-amber-500/40 bg-amber-500/10 shadow-[0_0_15px_rgba(245,158,11,0.1)]'
                        : '',
                    )}
                  >
                    <span
                      className={cn(
                        ...
                        isCreator ? 'text-amber-400' : 'text-white/70',
                      )}
                    >
                      {author?.username ? `@${author.username}` : 'Member'}
                      {isCreator && (
                        <Star className="inline w-3 h-3 ml-1 mb-0.5 fill-amber-400 text-amber-400" />
                      )}
                    </span>
      ...
      {isTipModalOpen && (
        <div className="absolute inset-0 z-50">
          <TipModal creatorId={username} onClose={() => setIsTipModalOpen(false)} />
        </div>
      )}
    </div>
  )
