    ...
    return (
      <div className={cn('clipcut-section-header mb-5', className)}>
        ...
        <div>
          ...
          {subtitle && (
            <p className="text-base sm:text-lg text-muted-foreground mt-2 font-sans font-normal max-w-2xl">
              {subtitle}
            </p>
          )}
        </div>
        {action && <div className="ml-auto flex-shrink-0 self-center">{action}</div>}
      </div>
    )
  }
  ...
  return (
    <div className={cn('flex items-start justify-between gap-4', className)}>
      <div className="flex flex-col gap-0.5">
        ...
        {size === 'sm' && <h3 className="text-base font-semibold text-foreground">{title}</h3>}
