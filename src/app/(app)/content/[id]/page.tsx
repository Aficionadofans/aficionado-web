...
async function getContent(id: string): Promise<{ content: ContentWithProfile | null; authorized: boolean }> {
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-base max-w-2xl">{content.description || 'No description provided.'}</p>
        </div>
        
        {profile && (
          <Link href={`/${profile.username}`} className="flex items-center gap-3 shrink-0 p-2 pr-4 rounded-full bg-surface/50 border border-border/50 hover:bg-surface transition-colors group">
            ...
            <div className="flex flex-col">
              <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Creator</span>
        ...
        {!authorized ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-gradient-to-t from-background/90 to-background/40 backdrop-blur-md">
            ...
            <p className="text-muted-foreground max-w-md mb-6">
              This video is exclusively for subscribers of <span className="text-off-white font-medium">@{profile?.username || 'this creator'}</span>. Subscribe to unlock this and all other premium content.
            </p>
            {profile?.username ? (
              <Link href={`/${profile.username}`} className="inline-flex items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors shadow-lg shadow-primary/25">
                View Profile to Subscribe
              </Link>
            ) : (
              <button disabled className="inline-flex items-center justify-center rounded-full bg-surface px-6 py-3 text-sm font-medium text-muted-foreground cursor-not-allowed">
